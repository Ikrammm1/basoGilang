...
const actions = {
  async list({ commit }) {
    try{
      const response = await axios.post('/api/packaging/list', {
        headers: {
  ...
  async process({ commit }, data) {
    ...
    return new Promise((resolve, reject) => {
      axios.post("/api/packaging/" + transactionType, data.formData, {
        headers: {
          'Content-Type': 'multipart/form-data'
        }
      })
  ...
  delete({commit}, data){
    return new Promise((resolve, reject)=>{
      axios.delete(`/api/packaging/delete/${data.id}`)
      .then((response)=>{
  ...
  newPass({commit}, data){
    return new Promise((resolve,reject)=>{
      axios.post("/api/packaging/newPass/" , data.formData, {
        headers: {
            'Content-Type': 'multipart/form-data'
        }
      }).then((response)=>{
  ...
  upload({commit}, data){
     // try {
      return new Promise((resolve,reject)=>{
        axios.post("/api/packaging/upload/" , data.formData, {
          headers: {
              'Content-Type': 'multipart/form-data'
          }
        }).then((response)=>{
