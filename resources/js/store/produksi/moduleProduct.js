...
const actions = {
  ...
  async process({ commit }, data) {
    ...
    return new Promise((resolve, reject) => {
      axios.post("/api/product/" + transactionType, data.formData, {
        headers: {
          'Content-Type': 'multipart/form-data'
        }
      })
  ...
  delete({commit}, data){
    return new Promise((resolve, reject)=>{
      axios.delete(`/api/product/delete/${data.id}`)
      .then((response)=>{
  ...
  newPass({commit}, data){
    return new Promise((resolve,reject)=>{
      axios.post("/api/product/newPass/" , data.formData, {
        headers: {
            'Content-Type': 'multipart/form-data'
        }
      }).then((response)=>{
  ...
  upload({commit}, data){
     // try {
      return new Promise((resolve,reject)=>{
        axios.post("/api/product/upload/" , data.formData, {
          headers: {
              'Content-Type': 'multipart/form-data'
          }
        }).then((response)=>{
