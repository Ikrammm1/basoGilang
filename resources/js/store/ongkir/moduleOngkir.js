...
const actions = {
  ...
  async process({ commit }, data) {
    ...
    return new Promise((resolve, reject) => {
      axios.post("/api/ongkir/" + transactionType, data.formData, {
        headers: {
          'Content-Type': 'multipart/form-data'
        }
      })
  ...
  delete({commit}, data){
    return new Promise((resolve, reject)=>{
      axios.delete(`/api/ongkir/delete/${data.id}`)
      .then((response)=>{
  ...
  newPass({commit}, data){
    return new Promise((resolve,reject)=>{
      axios.post("/api/ongkir/newPass/" , data.formData, {
        headers: {
            'Content-Type': 'multipart/form-data'
        }
      }).then((response)=>{
  ...
  upload({commit}, data){
     // try {
      return new Promise((resolve,reject)=>{
        axios.post("/api/ongkir/upload/" , data.formData, {
          headers: {
              'Content-Type': 'multipart/form-data'
          }
        }).then((response)=>{
