...
<template>
    ...
    <div v-if="sales.length !=0">
         <VRow class="mt-5 mb-3 ml-3">
            <h2>
                Total Pemasukan : {{ totalSalesPrice }}
            </h2>
        </VRow>
...
export default {
    ...
    computed:{
        ...
        sales() {
            ...
            const data = this.$store.state.report.sales;
        ...
        totalSalesPrice() {
            ...
            const result = this.sales.reduce((sum, item) => {
    ...
    methods:{
       formatTanggal(datetime) {
        ...
        resetForm(){
            ...
            const data = ''
            this.$store.dispatch("report/getReport", { data })
        ...
        submitForm(){
            ...
            formData.append('jenisReport', this.jenisReport);
            ...
            this.isLoading = true
             this.$store.dispatch("report/getReport", { formData })
                .then((response) => {
      ...
      exportReportToExcel() {
            ...
            if (this.jenisReport == 1) {
                ...
                'Total Harga (Rp)': this.formatRupiah(item.total_price),
                }));
                ...
                'Kategori': '',
                'Total Qty': 'TOTAL',
                'Total Harga (Rp)': this.totalSalesPrice, // angka asli dulu ya
                });
                ...
                'Kategori': item.customer_category,
                'Telp': item.telp,
                ...
                'Total Belanja (Rp)': this.formatRupiah(item.total_price),
                }));
                ...
                'Alamat': '',
                'Total Order': 'TOTAL',
                'Total Belanja (Rp)': this.totalSalesPrice,
                });
            }
            ...
            // 3. Simpan file
            const fileName = this.jenisReport == 1
                ?  `Laporan-Penjualan-Produk-${new Date().toISOString().slice(0, 10)}.xlsx`
                :  `Laporan-Transaksi-Customer-${new Date().toISOString().slice(0, 10)}.xlsx`
