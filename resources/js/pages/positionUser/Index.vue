...
<template>
    ...
    <div>
        ...
        <CustomTable
            ...
            :search="search"
        >
            <template #item.mp_code="{ item }">
                <span>{{ item.mp_code ? item.mp_code : '-' }}</span>
            </template>
            <template #item.mp_name="{ item }">
                <span>{{ item.mp_name ? item.mp_name : '-' }}</span>
            </template>
            <template #item.mp_desc="{ item }">
                <span>{{ item.mp_desc ? item.mp_desc : '-' }}</span>
            </template>
            <!-- Actions -->
...
export default {
    ...
    computed:{
        ...
        datas() {
            ...
            const data = this.$store.state.position.datas;
            ...
            // console.log(id)
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        // console.log("success")
                        this.$store.dispatch("position/delete", {id})
                        .then((response) =>{
                            this.notify = true
                            this.status = 'success'
                            this.title = 'Success!'
    ...
    async created() {
        this.isLoading = true
        await this.$store.dispatch("position/list")
        // this.$refs.table
