    ...
    <div>
        ...
        >
            ...
            <template #item.dateIn="{ item }">
                <span>{{ formatTanggal(item.updated_at || item.created_at) }}</span>
            ...
            <template #item.category="{ item }">
                <span>{{ item.product ? item.product.category.name : '-' }}</span>
            </template>
...
export default {
    ...
    computed:{
        ...
        datas() {
            ...
            const data = this.$store.state.stock.datas;
    ...
    methods:{
        ...
        async addNewData() {
            ...
            this.sidebarData = {};
            this.from = 'Tambah';
            await this.$store.dispatch("bahanBaku/listStock")
            await this.$store.dispatch("product/list")
            await this.$store.dispatch("stock/list")
            ...
            // console.log(id)
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        // console.log("success")
                        this.$store.dispatch("stock/delete", {id})
                        .then((response) =>{
                            this.notify = true
                            this.status = 'success'
                            this.title = 'Success!'
