...
<template>
    ...
    <div>
        ...
        >
            ...
            <template #item.outDate="{ item }">
                <span>{{ item.outDate ? formatTanggal(item.outDate) : '-' }}</span>
            </template>
            <template #item.bahan="{ item }">
                <span>{{ item.bahan_kemasan ? item.bahan_kemasan.name : '-' }}</span>
            </template>
...
export default {
    ...
    methods:{
        ...
        async addNewData() {
            ...
            this.sidebarData = {};
            this.from = 'Tambah';
            await this.$store.dispatch("bahanKemasan/listStock")
            ...
            // console.log(id)
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        // console.log("success")
                        this.$store.dispatch("bahanKemasan/deleteLog", {id})
                        .then((response) =>{
                            this.notify = true
                            this.status = 'success'
                            this.title = 'Success!'
    ...
    async created() {
        this.isLoading = true
        await this.$store.dispatch("bahanKemasan/listLog")
        await this.$store.dispatch("bahanKemasan/list")
