    ...
    <div>
        ...
        >
            ...
            <template #item.radius="{ item }">
                <span>{{ item.radius ? item.radius : '-' }}</span>
            </template>
            <template #item.desc="{ item }">
                <span>{{ item.desc ? item.desc : '-' }}</span>
            </template>
            <template #item.price="{ item }">
                <span>{{ item.price ?formatRupiah(item.price) : 0 }}</span>
            </template>
            <!-- Actions -->
...
export default {
    ...
    computed:{
        ...
        datas() {
            ...
            const data = this.$store.state.ongkir.datas;
            ...
            // console.log(id)
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        // console.log("success")
                        this.$store.dispatch("ongkir/delete", {id})
                        .then((response) =>{
                            this.notify = true
                            this.status = 'success'
                            this.title = 'Success!'
